...
export const postLoginUser = async (data) => {
  try {
    ...
    throw new Error(`Error al loggear al usuario: ${error.response.data.message || error.message}`);
  }
};
...
export const postRegisterUser = async (data) => {
  try {
    ...
    throw new Error(`Error al registrar al usuario: ${error.response.data.message || error.message}`);
  }
};